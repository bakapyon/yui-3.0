...
*
* Construído por Lucas R. - KillovSky, agradecimentos especiais ao grupo Horizon.
*
...
*
* 						Obrigado a você que escolheu a Yui.
*
* ---------------------------------------------------------------------------------------------------*/
...
module.exports = kconfig = async (kill, message) => {
    ...
    try {
        ...
        if (isGroupMsg && autoSticker && isMedia && isImage && !isCmd) {
            ...
            await kill.sendImageAsSticker(from, imageBase64, { author: 'Yui/Horizon', pack: '🎴 yui/Horizon ', keepScale: true })
        }
		...
		if (isGroupMsg && autoSticker && isMedia && isVideo && !isCmd) {
			...
			await kill.sendMp4AsSticker(from, videoBase64, null, { stickerMetadata: true, pack: '🎴 yui/Horizon ', author: 'Yui/Horizon', fps: 30, startTime: '00:00:00.0', endTime : '00:00:05.0', crop: true, loop: 0 })
		}
		...
		case 'f':	
            if (isMedia && isImage) {
                ...
                if (isUrl(url)) {
                    await kill.sendStickerfromUrl(from, url, { method: 'get' }, { author: 'Yui/Horizon', pack: '🎴 yui/Horizon ', keepScale: true })
		...
		case 'ttp':
			...
			await kill.sendImageAsSticker(from, ttpst.data.result, { author: 'Yui/Horizon', pack: '🎴 yui/Horizon ', keepScale: true })
			break
		...
		case 'attp':
			...
			await axios.get(`https://api.xteam.xyz/attp?file&text=${encodeURIComponent(body.slice(6))}`, { responseType: 'arraybuffer' }).then(async (response) => {
				...
				await kill.sendImageAsSticker(from, attp, { author: 'Yui/Horizon', pack: '🎴 yui/Horizon ', keepScale: true })
			})
			break
        ...
        case 'trigger':
            if (isMedia && type === 'image' || isQuotedImage) {
				...
				await axios.get(`https://some-random-api.ml/canvas/triggered?avatar=${getTrigger}`, { responseType: 'arraybuffer' }).then(async (response) => {
					...
					await kill.sendImageAsSticker(from, theTigger, { author: 'Yui/Horizon', pack: '🎴 yui/Horizon ', keepScale: true })
				})
				.catch(() => { kill.reply(from, mess.upfail(), id) })
            } else return kill.reply(from, mess.onlyimg(), id)
            break
		...
		case 'about':
			await kill.sendFile(from, './lib/media/img/yui.png', 'yui.png', mess.about(), id)
			break
        ...
        case 'nobg':
			if (isMedia && type === 'image' || isQuotedImage) {
				...
				await kill.sendImageAsSticker(from, `data:${mimetype};base64,${result.base64img}`, { author: 'Yui/Horizon', pack: '🎴 yui/Horizon ', keepScale: true })
		...
		case 'g':	
			if (isMedia && isVideo || isGif || isQuotedVideo || isQuotedGif) {
				...
				await kill.sendMp4AsSticker(from, mediaData, null, { stickerMetadata: true, pack: '🎴 yui/Horizon ', author: 'Yui/Horizon', fps: 10, startTime: '00:00:00.0', endTime : '00:00:05.0', crop: true, loop: 0 })
        ...
        case 'getsticker':
			...
			await kill.sendStickerfromUrl(from, stkfm, { method: 'get' }, { author: 'Yui/Horizon', pack: '🎴 yui/Horizon ', keepScale: true })
            break
		...
		case 'discord':
			...
			await kill.sendLinkWithAutoPreview(from, 'https://discord.gg/9zwrR75', 'S2')
			break
       ...
       case 'translate':
        ...
        case 'yui':
			...
			try {
				if (args[0] == '-g') {
					...
					const yui = await axios.get(`http://simsumi.herokuapp.com/api?text=${encodeURIComponent(body.slice(6))}&lang=${config.lang}`)
					if (yui.data.success == 'Limit 50 queries per hour.' || yui.data.success == '' || yui.data.success == null) {
						await kill.reply(from, resmf, id)
					} else {
						if (yui.data.success == 'Curta a pagina Gamadas por Bieber no facebook ;)') return kill.reply(from, 'Oi sua gostosa, como vai?', id)
						await kill.reply(from, yui.data.success, id)
						fs.appendFile('./lib/config/Utilidades/reply.txt', `\n${yui.data.success}`)
					}
				}
			} catch (error) { 
				await kill.reply(from, resmf, id)
				console.log(color('[yui]', 'crimson'), color(`→ Obtive erros no comando ${prefix}${command} → ${error.message} - Você pode ignorar.`, 'gold'))
			}
			break
        ...
        case 'speak':
			...
			try {
				if (args[0] == '-g') {
					...
					const spyui = await axios.get(`http://simsumi.herokuapp.com/api?text=${encodeURIComponent(body.slice(7))}&lang=${config.lang}`)
					const a = spyui.data.success
        ...
        case 'roll':
            ...
            await kill.sendStickerfromUrl(from, 'https://www.random.org/dice/dice' + dice + '.png', { method: 'get' }, { author: 'Yui/Horizon', pack: '🎴 yui/Horizon ', keepScale: true })
            break
        ...
        case 'flip':
            ...
            if (limitfp !== undefined && cd - (Date.now() - limitfp) > 0) {
				...
				if (args[0] == 'cara' || args[0] == 'coroa') {
					if (side == 1) {
						await kill.sendStickerfromUrl(from, 'https://i.ibb.co/LJjkVK5/heads.png', { method: 'get' }, { author: 'Yui/Horizon', pack: '🎴 yui/Horizon ', keepScale: true })
						if (args[0] == 'cara') {
						...
						await kill.sendStickerfromUrl(from, 'https://i.ibb.co/wNnZ4QD/tails.png', { method: 'get' }, { author: 'Yui/Horizon', pack: '🎴 yui/Horizon ', keepScale: true })
						if (args[0] == 'coroa') {
       ...
       case 'ins':
		...
		case 'ball':
			...
			await kill.sendStickerfromUrl(from, ball.data.url, { method: 'get' }, { author: 'Yui/Horizon', pack: '🎴 yui/Horizon ', keepScale: true })
			break
		...
		case 'cafune':
			...
			await axios.get(cfnean.data.url, { responseType: 'arraybuffer' }).then(async (response) => {
				...
				await kill.sendImageAsSticker(from, cfune, { author: 'Yui/Horizon', pack: '🎴 yui/Horizon ', keepScale: true })
			})
			break			
		...
		case 'poke':
			...
			await axios.get(pokean.data.url, { responseType: 'arraybuffer' }).then(async (response) => {
				...
				await kill.sendImageAsSticker(from, teco, { author: 'Yui/Horizon', pack: '🎴 yui/Horizon ', keepScale: true })
			})
			break
		...
		case 'cocegas':
			...
			await axios.get(cocegas.data.url, { responseType: 'arraybuffer' }).then(async (response) => {
				...
				await kill.sendImageAsSticker(from, cosqha, { author: 'Yui/Horizon', pack: '🎴 yui/Horizon ', keepScale: true })
			})
			break
		...
		case 'food':
			...
			await axios.get(feed.data.url, { responseType: 'arraybuffer' }).then(async (response) => {
				...
				await kill.sendImageAsSticker(from, gfood, { author: 'Yui/Horizon', pack: '🎴 yui/Horizon ', keepScale: true })
			})
			break
		...
		case 'baka':
			...
			await axios.get(baka .data.url, { responseType: 'arraybuffer' }).then(async (response) => {
				...
				await kill.sendImageAsSticker(from, bakay, { author: 'Yui/Horizon', pack: '🎴 yui/Horizon ', keepScale: true })
			})
			break
       ...
       case 'clima':
        ...
        case 'broad':
			...
			if (args[0] == '-all') {
				...
				for (let ids of chatz) {
					...
					if (!cvk.isReadOnly) {
						await kill.sendText(ids, `[Transmissão do dono da Yui]\n\n${msg}`)
						if (quotedMsgObj) {
				...
				for (let bclst of chatz) {
					...
					if (!notgps) {
						...
						if (!bkgps.isReadOnly) {
							await kill.sendText(bclst, `[Transmissão do dono da Yui]\n\n${msg}`)
							if (quotedMsgObj) {
								let replyOnReply = await kill.getMessageById(quotedMsgObj.id)
								let obj = replyOnReply.quotedMsgObj
								if (/ptt|audio|video|image|document|sticker/.test(quotedMsgObj.type)) {
        ...
        case 'hug':
			...
			await axios.get(chug.data.url, { responseType: 'arraybuffer' }).then(async (response) => {
				...
				await kill.sendImageAsSticker(from, hugsz, { author: 'Yui/Horizon', pack: '🎴 yui/Horizon ', keepScale: true })
			})
			break
        ...
        case 'boquete':
			...
			await axios.get(blowjob.data.url, { responseType: 'arraybuffer' }).then(async (response) => {
				...
				await kill.sendImageAsSticker(from, bjanime, { author: 'Yui/Horizon', pack: '🎴 yui/Horizon ', keepScale: true })
			})
			break
        ...
        case 'feet':
			...
			await axios.get(feet.data.url, { responseType: 'arraybuffer' }).then(async (response) => {
				...
				await kill.sendImageAsSticker(from, pezinime, { author: 'Yui/Horizon', pack: '🎴 yui/Horizon ', keepScale: true })
			})
			break
        ...
        case 'hard':
			...
			await axios.get(hard.data.url, { responseType: 'arraybuffer' }).then(async (response) => {
				...
				await kill.sendImageAsSticker(from, spank, { author: 'Yui/Horizon', pack: '🎴 yui/Horizon ', keepScale: true })
			})
			break
        ...
        case 'boobs':
			...
			await axios.get(bobis.data.url, { responseType: 'arraybuffer' }).then(async (response) => {
				...
				await kill.sendImageAsSticker(from, tetbobs, { author: 'Yui/Horizon', pack: '🎴 yui/Horizon ', keepScale: true })
			})
			break
        ...
        case 'lick':
			...
			await axios.get(lick.data.url, { responseType: 'arraybuffer' }).then(async (response) => {
				...
				await kill.sendImageAsSticker(from, lingani, { author: 'Yui/Horizon', pack: '🎴 yui/Horizon ', keepScale: true })
			})
			break
        ...
        case 'masturb':
			...
			await axios.get(mstbra.data.url, { responseType: 'arraybuffer' }).then(async (response) => {
				...
				await kill.sendImageAsSticker(from, twodedo, { author: 'Yui/Horizon', pack: '🎴 yui/Horizon ', keepScale: true })
			})
			break
        ...
        case 'anal':
			...
			await axios.get(solog.data.url, { responseType: 'arraybuffer' }).then(async (response) => {
				...
				await kill.sendImageAsSticker(from, anlnime, { author: 'Yui/Horizon', pack: '🎴 yui/Horizon ', keepScale: true })
			})
			break        
		...
		case 'randomloli':
			...
			await kill.sendImageAsSticker(from, loliz.data.url, { author: 'Yui/Horizon', pack: '🎴 yui/Horizon ', keepScale: true })
			break
        ...
        case 'nsfwicon':
			...
			await kill.sendImageAsSticker(from, icon.data.url, { author: 'Yui/Horizon', pack: '🎴 yui/Horizon ', keepScale: true })
			break
		...
		case 'icon':
			...
			await kill.sendImageAsSticker(from, avatarz.data.url, { author: 'Yui/Horizon', pack: '🎴 yui/Horizon ', keepScale: true })
			break
			
			
		case 'face':
			const gasm = await axios.get('https://nekos.life/api/v2/img/gasm')
			await kill.sendImageAsSticker(from, gasm.data.url, { author: 'Yui/Horizon', pack: '🎴 yui/Horizon ', keepScale: true })
			break
        ...
        case 'ihentai':
			...
			await axios.get(hentai1.data.url, { responseType: 'arraybuffer' }).then(async (response) => {
				...
				await kill.sendImageAsSticker(from, hntimg, { author: 'Yui/Horizon', pack: '🎴 yui/Horizon ', keepScale: true })
			})
            break
        ...
        case 'randomneko':
			...
			await axios.get(nekons.data.url, { responseType: 'arraybuffer' }).then(async (response) => {
				...
				await kill.sendImageAsSticker(from, gatadlc, { author: 'Yui/Horizon', pack: '🎴 yui/Horizon ', keepScale: true })
			})
            break
        ...
        case 'look' :
			...
			await axios.get(smug.data.url, { responseType: 'arraybuffer' }).then(async (response) => {
				...
				await kill.sendImageAsSticker(from, piscaeye, { author: 'Yui/Horizon', pack: '🎴 yui/Horizon ', keepScale: true })
			})
            break
		...
		case 'kisu':
			...
			await axios.get(kisu.data.url, { responseType: 'arraybuffer' }).then(async (response) => {
				...
				await kill.sendImageAsSticker(from, beijaod, { author: 'Yui/Horizon', pack: '🎴 yui/Horizon ', keepScale: true })
			})
			break
		...
		case 'tapa':
			...
			await axios.get(tapi.data.url, { responseType: 'arraybuffer' }).then(async (response) => {
				...
				await kill.sendImageAsSticker(from, tapasso, { author: 'Yui/Horizon', pack: '🎴 yui/Horizon ', keepScale: true })
			})
			break
		...
		case 'kiss':
			if (isGroupMsg && args.length == 1 && mentionedJidList.length !== 0) {
				...
				await axios.get(kiss.data.url, { responseType: 'arraybuffer' }).then(async (response) => {
					...
					await kill.sendImageAsSticker(from, kissup, { author: 'Yui/Horizon', pack: '🎴 yui/Horizon ', keepScale: true })
				})
        ...
        case 'slap':
			if (isGroupMsg && args.length == 1 && mentionedJidList.length !== 0) {
				...
				await axios.get(tapa.data.url, { responseType: 'arraybuffer' }).then(async (response) => {
					...
					await kill.sendImageAsSticker(from, tapaol, { author: 'Yui/Horizon', pack: '🎴 yui/Horizon ', keepScale: true })
				})
		...
		case 'emoji':
			...
			emoji.get(args[0]).then(async (emoji) => {
				...
				await kill.sendStickerfromUrl(from, emoji.images[0].url, { method: 'get' }, { author: 'Yui/Horizon', pack: '🎴 yui/Horizon ', keepScale: true })
			})
			break
		...
		case 'cosplay':
			...
			const plate = await axios.get(`https://rest.farzain.com/api/special/fansign/cosplay/cosplay.php?apikey=rambu&text=${encodeURIComponent(body.slice(9))}`, { responseType: 'arraybuffer' }).then(async (response) => {
				...
				await kill.sendImageAsSticker(from, myplaye, { author: 'Yui/Horizon', pack: '🎴 yui/Horizon ', keepScale: true })
			})
			break
